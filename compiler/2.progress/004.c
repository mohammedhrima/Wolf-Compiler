...
Token *get_var(char *name)
{
    ...
    for (int i = 0; i < CURRENT_LABEL->pos; i++)
...
{
    if (func_pos + 10 > func_len)
    {
        Node **tmp = calloc(func_len * 2, sizeof(Node *));
        memcpy(tmp, FUNCTIONS, func_pos * sizeof(Node *));
        free(FUNCTIONS);
        FUNCTIONS = tmp;
        func_len *= 2;
    }
    debug("new function, name: %s, return type: %s\n", node->token->name, type_to_string(node->token->type));
    // print_node(node, 1);
    return (FUNCTIONS[func_pos++] = copy_node(node));
}
...
Node *get_func(char *name)
{
    ...
    for (int i = 0; i < func_pos; i++)
        if (FUNCTIONS[i]->token->name && strcmp(FUNCTIONS[i]->token->name, name) == 0)
            return FUNCTIONS[i];
    return NULL;
}
...
Token *new_variable(Token *token)
{
    ...
    if (CURRENT_LABEL->pos + 10 > CURRENT_LABEL->len)
    {
        Token **tmp = calloc(CURRENT_LABEL->len * 2, sizeof(Token *));
        memcpy(tmp, CURRENT_LABEL->variables, CURRENT_LABEL->pos * sizeof(Token *));
        free(CURRENT_LABEL->variables);
        CURRENT_LABEL->variables = tmp;
        CURRENT_LABEL->len *= 2;
    }
    debug("new variable %k in ptr: %zu\n", token, token->ptr);
    return (CURRENT_LABEL->variables[CURRENT_LABEL->pos++] = token);
}
        ...
        if (tmp_token = check(lparent_, 0))
        {
            ...
            */
            ...
            if (strcmp(node->token->name, "main") == 0)
            {
                ...
                ;
            else if (!get_func(node->token->name))
...
int main(int argc, char **argv)
{
    ...
    FUNCTIONS = calloc(func_len, sizeof(Node *));
