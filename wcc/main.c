...
void print_node(Node *node, int col)
{
    ...
    if (node)
    {
        ...
        case func_dec_:
        {
            ...
            // code bloc
            tmp = node->right;
            // TODO: verify that function get initlized by none_ ret_type
            while (tmp)
            {
                print_node(tmp->left, col + 1);
                tmp = tmp->right;
...
Node *assign()
{
    ...
    {
        // TODO: I made some modification, check this
        Node *right = assign();
        Node *tmp = right;
...
Node *prime()
{
    ...
    {
        // TODO: expect return if function has ret_type
    ...
    else if (token = check(ref_, 0))
    {
        ...
        if (!token->name)
            error(__LINE__, "expected a identifier after ref keyword");
        type = token->type;
    ...
    else if (token = check(return_, 0))
    {
        ...
        error(__LINE__, "Unexpected %s in prime", token ? tts(token->type) : "(null)");
    return node;
}
    ...
    */
    ...
    case bool_:
    {
        if (node->token->name)
        {
            if (get_var(node->token->name))
                error(__LINE__, "redefinition of variable in %s", __func__);
    ...
    case lbracket_:
    {
        ...
        if (left->type != array_ && left->type != lbracket_)
            error(__LINE__, "Expected array to iterate over %s", tts(left->type));
    ...
    case assign_:
    {
        ...
        if (!left->name ||
            ...
            error(__LINE__, "Invalid assignement %s / %s", tts(left->type), tts(right->type));
        ...
        */
        if (right->is_ref && !right->has_ref)
            error(__LINE__, "%s must have a reference before get used", right->name);
        if (!skip_check && left->is_ref && !left->has_ref && !right->name)
            error(__LINE__, "reference %s must be assigned to variable first time", left->name);

        node->token = left;
#if 0
        if (left->is_ref && right->is_ref)
        {
            error(__LINE__, "left is ref , right is ref\n");
            exit(1);
            print_asm("   mov     rax, QWORD PTR -%zu[rbp]\n", right->ptr);
        ...
        else
        {
            ...
            case array_:
            {
                ...
                if (right->ptr)
                {
                    ...
                    error(__LINE__, "handle this one %s", __func__);
                break;
            }
            default:
                error(__LINE__, "add assembly for this one 0, %s", __func__);
                break;
            }
        }
        visualize();
...
#define BOOL_PTR 0
    ...
    case func_call_:
    {
        ...
        else
        {
            ...
            if (dec->left)
            {
                ...
                Node *d_tmp = dec->left;
                Node *c_tmp = call->left; // TODO: protect it from SEGV
                ...
                while (d_tmp && c_tmp)
                {
                    ...
                    if (d_tmp->left->token->type != c_tmp->left->token->type)
                        error(__LINE__, "Uncompatible type in function call");
                    Token *token = c_tmp->left->token;
                    // TODO: to be checked
                    // token->is_ref = d_tmp->left->token->is_ref;
                    // stack[stack_pos++] = c_tmp->left->token;
                ...
                if (d_tmp)
                    error(__LINE__, "'%s' require arguments", call->token->name);
                stack_pos--;
                // push arguments
                c_tmp = call->left;
                while (stack_pos >= 0)
                    ...
                    else
                    {
                        ...
                        case char_:
                            if (token->index_)
                                ...
                                error(__LINE__, "something went wrong");
                            break;
                        case int_:
                            print_asm("    mov     rax, %ld\n", token->int_);
                            break;
    ...
    case func_dec_:
    {
        ...
        while (tmp)
        {
            ...
            /*
                TODO:
                    I did only test for (integers/char), check other data types
            */
        ...
        while (tmp)
        {
            ...
            if (tmp->left->token->type == return_)
            {
                ...
                // TODO check token type, this handeling is not enough
                if (token->ptr)
                    print_asm("   mov	  rax, QWORD PTR -%zu[rbp]\n", token->ptr);
            }
            else
    ...
    case break_:
    {
        ...
        if (lb_tmp == 0)
            error(__LINE__, "Expected a loop to do %s\n", tts(node->token->type));
        break;
    }
    default:
